	...
	if(count _num >1) then
		{
			...
			[10,[],{wsquadRP2 setVehiclePosition [(player modelToWorld[0,2,0]), [],0.5, "CAN_COLLIDE"]; hint "Rally Point Deployed"},{hint "Deploying interupted"},"Deploying Rally point"] call ace_common_fnc_progressBar
		} 
	else 
		{
